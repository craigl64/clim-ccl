...
(defmethod display-all-demos ((frame demo-driver) stream &rest args)
  ...
  (with-text-style (stream '(:serif :roman :large))
	...
	(let ((result (#-Allegro progn #+Allegro catch #+Allegro 'excl::printer-error
...
(defun run-demo (demo &key (port (find-port)) force background)
  ...
  (flet ((do-it ()
	...
	(#+Allegro mp:process-run-function 
         #+Clozure ccl:process-run-function
         #-(or Allegro Clozure) (error "Don't know how to start a process")
	 `(:name ,(demo-name demo)
