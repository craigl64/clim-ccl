...
(defmethod node-generate-inferior-objects ((node function-call-node) (type (eql ':callees)))
  (let ((function (node-object node)))
	...
	(loop for method in (sort (flavor:generic-function-methods
	      ...
	      when function
		collect function))
      (#-aclpc clos:generic-function
       #+(or aclpc Clozure) generic-function
	;; Return all the real methods for this generic function
	(loop for method in (clos-internals::sort-generic-function-methods
			      function
			      (copy-list (#-(or aclpc Clozure) clos:generic-function-methods 
                                          #+aclpc generic-function-methods 
                                          #+Clozure ccl:generic-function-methods
                                          function))
			      :sort :heuristic)
	      as function = (#-(or aclpc Clozure) clos:method-function
                             #+aclpc method-function
                             #+Clozure ccl:method-function
                             method)
	      when function
		collect function))
      (compiled-function
...
(defmethod node-generate-inferior-objects ((node function-call-node) (type (eql ':callers)))
  (let ((function (node-object node)))
	...
	(#-aclpc clos:method
         #+(or aclpc Clozure) method
	  (list (#-(or aclpc Clozure) clos:method-generic-function
                 #+aclpc method-generic-function
                 #+Clozure ccl:method-generic-function
	  ...
	  (dolist (item methods)
		...
		(destructuring-bind (gf method) item
		  (with-output-as-presentation (stream method 'expression
		    ...
		    (prin1 (#-(or aclpc Clozure) clos:generic-function-name 
                            #+aclpc generic-function-name
                            #+Clozure ccl:generic-function-name gf) stream)))))))))))
...
(defmethod print-object :around ((node lisp-object-call-node) stream)
  (handler-case (call-next-method node stream)
    (error (err) err (print-not-readable))))
...
(define-browser-command com-show-clos-slots
  ...
  (let* ((class (node-object class-node))
	 (slots (sort (mapcar #' #-(or aclpc Clozure) clos:slot-definition-name
                                 #+aclpc slot-definition-name
                                 #+Clozure ccl:slot-definition-name
                          (#-(or aclpc Clozure) clos:class-direct-slots
                           #+aclpc class-direct-slots
                           #+Clozure ccl:class-direct-slots
                           class))
		      #'string-lessp)))
    (when (and slots
...
(define-browser-command com-show-clos-methods
  ...
  (let* ((class (node-object class-node))
	 (methods (#-(or aclpc Clozure) clos:specializer-direct-methods
                   #+aclpc specializer-direct-methods
                   #+Clozure ccl:specializer-direct-methods
                   class))
	 (method-list
	   (loop for method in methods
		 collect (list (#-(or aclpc Clozure) clos:method-generic-function
                                #+aclpc method-generic-function
                                #+Clozure ccl:method-generic-function
                                method) method))))
    (setq method-list (sort method-list #'string-lessp
			    :key #'(lambda (item)
				     (let ((name (#-(or aclpc Clozure)
