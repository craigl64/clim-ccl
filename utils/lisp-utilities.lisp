...
(ccl:define-declaration non-dynamic-extent (spec env)
  (let ((vars (rest spec))
    ...
    (dolist (v vars)
      (block process-var
        (multiple-value-bind (type local info)
                             (ccl:variable-information v env)
          (declare (ignore local))
          (case type
            (:lexical
             (when (cdr (assoc 'dynamic-extent info))
...
(defun find-class-that-works (name &optional (errorp t) environment)
  ...
  #+ccl-2
  (when (eq environment 'compile-file)
    (setq environment ccl::*fcomp-compilation-environment*))
  #+Clozure
  (when (eql environment 'compile-file)
    (setf environment ccl::*fcomp-load-forms-environment*))
  #+Allegro
