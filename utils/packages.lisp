...
;	#+(and allegro (not (version>= 9 0))) stream
        #+Clozure gray
        #+Clozure ccl
	#+aclpc allegro)

 ;; Shadow these symbols so that we can define methods for them.
 (:shadow pathname truename)

 ;; Shadow this everywhere to make it a generic function
 #-Clozure (:shadow interactive-stream-p)
...
(defpackage clim-silica
  ...
  (:export
    ...
    *all-character-sets*
    ...
    port-terminated
    port-trace-thing
    #+(or aclpc acl86win32 Clozure) port-undefined-text-style
