...
(defmacro holding-lock ((locator display &optional whostate &key timeout)
  ...
  `(mp:with-lock (,locator ,whostate ,timeout)
    ,@body))
...
(defvar *conditional-store-lock* (mp:make-lock :name "conditional store"))
...
(defun open-x-stream (host display protocol)
  ...
  (when (or (string= host "") (string= host "unix"))
    (setf host "localhost"))
  (comm:open-tcp-stream host (+ 6000 display)
    :direction :io
  ...
  (let ((stream (display-input-stream display)))
	...
	(progn
          (loop for i from start to end
            do (setf (aref vector i) (read-byte stream)))
...
(defun buffer-write-default (vector display start end)
  ...
  (let ((stream (display-output-stream display)))
    ...
    (unless (null stream)
      (loop for i from start to end
        do (write-byte (aref vector i) stream))
