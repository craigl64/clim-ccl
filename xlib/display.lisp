    ...
    (let ((family-id (read-short stream nil)))
      (if (null family-id)
          ...
          (let* ((address-data (read-short-length-vector stream))
                 (number (read-short stream)) ;;(parse-integer (read-short-length-string stream)))
  ...
  (when (null authorization-name)
    (multiple-value-setq (authorization-name authorization-data)
      (get-best-authorization host
				  ...
				  :local
				  protocol))))
  (format t "Auth Name: ~A~%" authorization-name)
  (format t "Auth Data: ~S~%" authorization-data)
...
;			   :authorization-data authorization-data)
	  ...
	  (initialize-resource-allocator disp)
          (format t "~&;; Resource Allocator Initialized~%")
	  (initialize-predefined-atoms disp)
          (format t "~&;; Predefined Atoms Initialized~%")
	  (initialize-extensions disp)
          (format t "~&;; Extensions Initializzed~%")
	  (setq ok-p t))
	  ...
	  (with-buffer-input (reply-buffer :sizes (8 16 32))
	    ...
	    (let ((success (boolean-get 0))
	      ...
	      (declare (ignore total-length))
              (format t "~&;; Display-Connect~%")
              (format t "~&;;   success: ~S~%" success)
              (format t "~&;;   major:   ~S~%" major-version)
              (format t "~&;;   minor:   ~S~%" minor-version)
	      (unless success
